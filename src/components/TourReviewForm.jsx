...
import { verifyCustomerBooking, submitTourReview } from "../lib/api";
  ...
  if (!isAuthenticated) {
    return (
      <div className="card soft" style={{ padding: "1.5rem" }}>
        <p style={{ margin: 0 }}>
          <Link to="/login">Sign in</Link> to leave a review after your tour.
        </p>
      </div>
    );
  }
    ...
    <form onSubmit={handleSubmit} className="card soft" style={{ padding: "1.5rem" }}>
      ...
      {!hasBooking && !bookingId && (
        <p style={{ color: "var(--text-soft)", fontSize: "0.9rem" }}>
          Reviews from guests who completed a booking are prioritized.
        </p>
      )}
